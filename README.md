...
극단적인 20개의 밸런스게임으로 골프 성향 4글자를 뽑아주는 웹앱입니다.
...
- 저장되는 값: 시각 · 유형 코드 · 20문항 응답 · 성별 · 나이대 · 구력 · 평균타수 · 목적
- **닉네임은 저장되지 않습니다.**
...
- `var QS = [` — 20개 문항(축당 5개). `ax`는 축, `av`/`bv`는 각 선택지가 가리키는 글자
- `var T = {` — 16유형 이름과 설명
- `function compat(` — 궁합 점수 규칙
- `var TIPS`, `var PURPOSE` — 평균타수별 팁 / 목적별 코멘트

문항을 더하거나 뺄 때는 **축당 문항 수를 홀수로 유지**하세요. 짝수가 되면 축이 50:50으로 갈리는 경우가 생깁니다.
`Code.gs`의 `NUM_Q`도 문항 수와 반드시 같아야 하고, 바꾸면 **기존 응답은 형식이 달라 집계에서 자동 제외**됩니다.
